...
public class HashTableWithTrace {
    ...
    public static final String TP_PROBING_START = "PROBING_START";
    public static final String TP_PROBING_NEXT = "PROBING_NEXT";
    ...
    public void insert(String key) {
        ...
        tracePoints.add(TP_PROBING_START);
        ...
        while (i < capacity) {
            if (table[index] == null || table[index].equals("DELETED")) {
                ...
                if (table[index].equals(key)) {
                    // Ключ уже существует - не вставляем повторно
                    tracePoints.add(TP_INSERT_SUCCESS + ":duplicate");
                    return;
                }
                ...
                tracePoints.add(TP_PROBING_NEXT + ":" + index);
            }
        }
    ...
    public void printTable() {
        ...
        for (int i = 0; i < capacity; i++) {
            System.out.println("  [" + i + "]: " + 
                (table[i] == null ? "null" : 
                 table[i].equals("DELETED") ? "DELETED" : table[i]));
        }
    }
}